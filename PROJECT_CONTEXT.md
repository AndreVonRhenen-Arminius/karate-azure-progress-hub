...
- Main purposes:
  ...
  - Notes, weekly reviews, progress and optional Supabase synchronisation
...
4. `js/config.js`, if it is added in a later project copy. This archive does not currently contain that file.
...
- `app.js` — programme definitions, adaptive priorities, mastery state, rendering, event handling, backups, timer and Supabase sync.
...
The application has no build step and no package dependencies. It loads Supabase JS as an ES module from jsDelivr only when cloud configuration exists.
...
4. Start the local server with `run-local.bat`.
5. Test desktop and phone widths.
6. Confirm Today displays one task, a kata focus and a grading focus.
7. Confirm an Azure recall result schedules a next review.
8. Confirm a lab journal updates the selected module evidence.
9. Confirm karate ratings change the recommended grading focus.
10. Confirm kata section changes update progress and retention actions set review dates.
11. Verify local progress survives a reload.
12. Verify an older JSON backup imports and receives state version 4.
13. Verify Supabase sign-in, pull and push without altering existing configuration.
14. Verify the installed PWA refreshes to cache version 1.6.0.

## Development priorities after 1.6.0
