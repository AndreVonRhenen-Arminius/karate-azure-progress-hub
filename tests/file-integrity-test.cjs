...
assert.ok(index.includes('app.js?v=1.9.3'), 'index must reference v1.9.3 app');
assert.ok(index.includes('styles.css?v=1.9.3'), 'index must reference v1.9.3 styles');
assert.notStrictEqual(index, msal, 'index.html must not contain the MSAL bundle');
assert.ok(app.includes("const APP_VERSION = '1.9.3'"), 'app version must be 1.9.3');
assert.ok(styles.includes(':root'), 'styles.css must contain CSS');
assert.ok(sw.includes("ka-progress-hub-v1.9.3"), 'service worker cache must be v1.9.3');
